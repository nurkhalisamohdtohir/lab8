...
public class ClientTextApplication extends Frame implements ActionListener,Runnable
{
 //Declarations
 Button b;
 TextField tf;
 ...
 public ClientTextApplication()
 {
  Frame f=new Frame("Client Side Words Calculator");//Frame for Client
  ...
  try{
   s=new Socket(InetAddress.getLocalHost(),4223);//Socket for client
 ...
 public void run()
 {
  ...
  {
   try{
	  ta.append("Number of words: " +br.readLine()+"\n");//Append to TextArea
   }catch(Exception e) {}
  }
 }
 ...
 {
  //Instantiate ClientTextApplication class
	 ClientTextApplication client = new ClientTextApplication();
 }
}